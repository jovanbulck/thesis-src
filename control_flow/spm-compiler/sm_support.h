 ...
 *
 * @note jo: added the following private vars per sm:
 *  __sm_sad    scheduler address
 *  __sm_sid    scheduler ID
 *  __sm_vep    scheduler violation entry point
 *  __sm_yep    scheduler yield entry point
 *  __sm_gep    scheduler get_cur_thr_id entry point
 *  __sm_thr    internal saved current thread id
 */
#define DECLARE_SM(name, vendor_id)                             \
    extern char __PS(name), __PE(name), __SS(name), __SE(name); \
    extern void*        __sm_##name##_sad;                      \
    extern sm_id        __sm_##name##_sid;                      \
    extern entry_idx    __sm_##name##_vep, __sm_##name##_yep,   \
                        __sm_##name##_gep;                      \
    extern unsigned     __sm_##name##_thr; \
...
always_inline sm_id sancus_get_self_id(void)
{
    ...
    //XXX get_self_id equals get_id(program_counter)
    asm("mov r0, %0"
    : "=m"(addr)
    );
