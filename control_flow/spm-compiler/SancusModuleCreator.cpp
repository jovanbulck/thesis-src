...
Function* SancusModuleCreator::getStub(Function& caller, Function& callee)
{
    ...
    else if (callerInfo.name.empty()) // call unprotected -> SM
    {
        ...
        stubAsm = Twine("\t.align 2\n"
                        ...
                        "\tpush r6\n"
                        "\tpush r7\n"
                        // jo: r8 is caller-save now as it holds the magic busy
                        // value on return
                        "\tpush r8\n"
                        "\tmov #" + idxName + ", r6\n"
                        "\tmov #1f, r7\n"
                        ...
                        /**
                         * jo: FIXME the stub should check here whether r8 contains
                         *  the magic busy value and yield to the scheduler 
                         *  returning here if so...
                         */
                        // jo: restore caller-save register r8
                        "\tpop r8\n"
                        "\tpop r7\n"
                        "\tpop r6\n"
                        "\tret"         // continue unprotected execution
                       ).str();
